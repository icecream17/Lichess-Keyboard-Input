setTimeout(function() {
   /*
      ...
      sName --> someonesUsername
      nameC --> nameCollection
   */

   var cx, cy,
      doit = 0,
      someonesUsername; // Changed variable name
   // Sorry if that incoveninces you, but I have
   // ctrl+d find&replace autocomplete

   // Gets the username if logged in
   // Note: Changed != undefined to !== null
   if (document.getElementById("user_tag") !== null) {
      var name1 = document.getElementById("user_tag").innerText;
   }

   // If you're logged in and playing a game, your username will show up.
   // Also your opponent, if you're not in zen mode.
   // Otherwise this is empty.
   // Changed variable name
   var nameCollection = document.getElementsByClassName("text ulpt");
   // var name2=document.getElementsByClassName("text ulpt")[0];

   // For each username that is playing....
   for (var i = 0; i < nameCollection.length; i++) {
      console.log("@"); // Probably for debug or something

      // if the username has a space...
      // Note that usernames cant have a space. The only way for this to be
      // true, is that the username has a title, like "LM Lance5500"
      // If that's the case, just select the second one.
      if (nameCollection[i].innerText.indexOf(' ') !== -1) {
         someonesUsername = nameCollection[i].innerText.split(" ")[1]
      } else {
         /*
            let something = nameCollection[i].innerText;
            do {
               let i = something.indexOf(' ') // -1 cuz no spaces
               something = something.slice(i + 1) // does nothing
               someonesUsername = something
            }
            while (something.indexOf(' ') !== -1) // quits afterwards
         */

         // I commented out the above code.
         // Spaces were already checked in the if statement.z
         someonesUsername = nameCollection[i].innerText
      }

      console.log(name1, someonesUsername);

      // Changed "==" to "==="
      // If the person logged in is the one playing the game, ACTIVATE
      if (name1 === someonesUsername) {
         doit = 1;
      }
   }

   // If you're logged out there's a signin button
   // changed != undefined to !== null
   if (document.getElementsByClassName("signin button button-empty")[0] !== null) {
      var anonc = document.getElementsByClassName("signin button button-empty")[0];

      // Hmm.
      if (anonc.innerText === "SIGN IN") doit = 1;
   }
   ...
   if (doit) {
      ...
      console.log(document.cookie); // Logs the cookies.
      var exdays = 1000; // Days until cookies expire.
      ...
      // self explanatory
      // returns "" when cookie doesn't exist
      ...
      */
      var keyORnot, KeyCO = getCookie("KeyB");

      /* If cookie exists set keyORnot to cookie. Else: 1*/
      if (KeyCO != "") {
      ...
      */
      ...
      // (suggestion) <details> tag cuz of show / hide function.
      // or <form> tag because of inputs?
      // Wow. It feels weird to work with someone else's code. Ive never done
      // this before. I wonder how I would feel if someone messed with my Code

      // Probably its horrifying or something
      container.innerHTML = `
         <div id="container">
            <button id="KeyboardO">Show</button>
            <div id="pieces">
               <label>Pawn Up :</label>
               <input type="text" value="w" id="Kpawnu" class="keyB">
               <label>Pawn ⇖ :</label>
               <input type="text" value="q" id="Kpawnl" class="keyB">
               <label>Pawn ⇗ :</label>
               <input type = "text" value="e" id="Kpawnr" class="keyB">
               <label>Queen :</label>
               <input type="text" value="s" id="Kqueen" class="keyB">
               <label>Rook :</label>
               <input type="text" value="1" id="Krookl" class="keyB">
               <label>Rook ▣ :</label>
               <input type="text" value="3" id="Krookr" class="keyB">
               <label>Knight :</label>
               <input type="text" value="a" id="Kknightl" class="keyB">
               <label>Knight ▣ :</label>
               <input type="text" value="d" id="Kknightr" class="keyB">
               <label>Bishop :</label>
               <input type="text" value="Space" id="Kbishop" class="keyB">
               <label>King :</label>
               <input type="text" value="shift" id="Kking" class="keyB">
               <label><button id="Save">Save</button></label>
               <summary> Hover the cursor over a destination square and press a corresponding key* </summary>
               <!-- Commented out, html comments
                  <a href="https://ultrabullet.com/keyboard" id="readm" target="_blank">*Read more</a>
               -->
            </div>
         </div>`
      ...
      var button1 = document.getElementById("KeyboardO"); // hide/show
      //  var container = document.createElement("div");
      var pieces = document.getElementById("pieces"); // see above
      //  pieces.id = "pieces";
      ...
      // also removes/adds eventlistener
         ...
         /* if (oneKey() == true)
{
    ...
    document.body.addEventListener("mousemove", MouseMove, false);
      ...
      /* Changes text and sets cookies. (assignkeys) */
      ...
      // (above) Changes backgroundcolor of piece when keyboard
      // (below) Makes the chat work? (i didnt really look)
      // (even lower) Updates text?
      ...
      /* new Set? i guess searching things up is common...er than i thought */
      ...
      // Mwahahaha!
      // I've decoded the base64!
      // Oh wait. What is this? I know nothing about xml....
      ...
      // I guess this might be where you fix my visual want?
      // Oh. That would be a lot of drawings and pictures. Hmm.
      // Sigh. I guess it's unlikely.
      if (mycolor == "black") {
         nn0 = 1;
         nn1 = 0;
         if (document.getElementsByClassName(mycolor + " rook")[nn1] != undefined)
      ...
      /* My brain is too wracked so i wont try to understand any
      more of the code */
